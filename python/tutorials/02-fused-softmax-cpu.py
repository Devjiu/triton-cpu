...
@triton.jit
def softmax_kernel_pers(output_ptr, input_ptr, input_row_stride, output_row_stride, n_rows, n_cols, TILE_SIZE: tl.constexpr):
    # The rows of the softmax are independent, so we parallelize across those
...
def benchmark(M, N, provider):
    ...
    if provider == 'torch-cpu-compile':
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: compiled(x, y), quantiles=quantiles, is_cpu=is_cpu)
    ...
    if provider == 'triton-cpu-orig-nomvec':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: softmax_orig(x, y), quantiles=quantiles, is_cpu=is_cpu)
    ...
    if provider == 'triton-cpu-tiled-pers-mvec':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: softmax_pers(x, y), quantiles=quantiles, is_cpu=is_cpu)

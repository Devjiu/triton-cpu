...
def add(x: torch.Tensor, y: torch.Tensor, output: torch.Tensor, is_cpu):
    ...
    # block_size = max(n_elements / 64, 32762)
    ...
    add_kernel[grid](x, y, output, n_elements, BLOCK_SIZE=CPU_BLOCK_SIZE, TILE_SIZE=CPU_TILE_SIZE)
    #else:
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=[2**i for i in [21]], #range(, 14, -1)],  # Different possible values for `x_name`.

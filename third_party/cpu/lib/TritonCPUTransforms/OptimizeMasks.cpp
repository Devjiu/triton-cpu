...
// to
//   val / K
// if N % K == 0 and val is not negitive. Usually, we cannot prove value to be
...
bool isAlwaysAllOnes(Value mask) {
  ...
  if (diff.getKind() == AffineExprKind::Constant) {
    int64_t diffVal = cast<AffineConstantExpr>(diff).getValue();
